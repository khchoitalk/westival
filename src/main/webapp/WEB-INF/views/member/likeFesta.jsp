...
<script type="text/javascript">
	...
	function myLikeFestaSearch(page){
			...
			$.ajax({
				...
				success : function(result){
					...
					if(jsonObj.currentPage <= 1)
						...
						$("#pagenation").append("<li><a href='javascript:myLikeFestaSearch(" + 1 + ")'>«</a></li>");
					
					for(var p = jsonObj.startPage; p <= jsonObj.endPage; p++) {
						if(p == jsonObj.currentPage)
							$("#pagenation").append("<li disabled><a style='font-weight: bold;' disabled>" + p + "</a></li>");
						else
							$("#pagenation").append("<li><a href='javascript:myLikeFestaSearch(" + p + ")'>" + p + "</a></li>");
					}
					
					if(jsonObj.currentPage >= jsonObj.maxPage)
						$("#pagenation").append("<li><a disabled>»</a></li>");
					else
						$("#pagenation").append("<li><a href='javascript:myLikeFestaSearch(" + jsonObj.maxPage + ")'>»</a></li>");
				},
				error : function(request, status, errorData){
					alert("error code : " + request.status + "\n" + "message : " + request.responseText 
							+ "\n" + "error : " + errorData);
	...
	function myFestaSearchMonth(month, page){	
		$.ajax({
			...
			success : function(result){
				...
				if(jsonObj.currentPage <= 1)
					...
					$("#pagenation").append("<li><a href='javascript:myFestaSearchMonth(" + jsonObj.month + ", " + 1 + ")'>«</a></li>");
				
				for(var p = jsonObj.startPage; p <= jsonObj.endPage; p++) {
					if(p == jsonObj.currentPage)
						$("#pagenation").append("<li><a style='font-weight: bold;' disabled>" + p + "</a></li>");
					else
						$("#pagenation").append("<li><a href='javascript:myFestaSearchMonth(" + jsonObj.month + ", " + p + ")'>" + p + "</a></li>");
				}
				
				if(jsonObj.currentPage >= jsonObj.maxPage)
					$("#pagenation").append("<li><a>»</a></li>");
				else
					$("#pagenation").append("<li><a href='javascript:myFestaSearchMonth(" +  jsonObj.month + ", " + jsonObj.maxPage + ")'>»</a></li>");
			},
			error : function(request, status, errorData){
				alert("error code : " + request.status + "\n" + "message : " + request.responseText 
						+ "\n" + "error : " + errorData);
...
<div class="super_container">
   ...
   <div class="home" id="home">
      <div class="home_background parallax-window" data-parallax="scroll" data-image-src="/westival/resources/images/elements_background.jpg"></div>
      <div class="home_content">
         <div class="home_title">관심축제</div>
      </div>
   </div>
   ...
   <div class="container">
		...
		<!-- search bar -->
		<div id="select_month">
			<button type="button" class="btn" id="1month_button" onclick="myFestaSearchMonth(1, 1);">1개월</button>
			<button type="button" class="btn" id="3month_button" onclick="myFestaSearchMonth(3, 1);">3개월</button>
			<button type="button" class="btn" id="6month_button" onclick="myFestaSearchMonth(6, 1);">6개월</button>
			<div id="search_month">
				<input type="date" class="YY-MM-dd" id="start_date">
				<span>~</span>
				<input type="date" class="YY-MM-dd" id="end_date">
				<button type="button" class="btn" onclick="myFestaSearch(1);">검색</button>
			</div>
		</div>
